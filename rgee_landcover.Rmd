...
stack_eviroment <- ee$Image(c(nldc,impervious,tree_cover,ndvi,tmax,tmin))
```
...
```{r,  message=FALSE}
standar_enviroment <- img_to_stand(x = stack_eviroment) 
enviroment_index <- pca_rgee(x = standar_enviroment) 
viz <- list(
  bands = c('pc3','pc2','pc1'),
  min = -2.5 ,
  max = 2.5
...
Map$addLayer(enviroment_index,visParams = viz,name = 'pc3-pc2-pc1')
```
...
```{r ,  message=FALSE}
source('utils.R')
(table <- impor_pca(x = standar_enviroment))
...
stack_tobands <- enviroment_index %>% ee$Image$toDouble()
